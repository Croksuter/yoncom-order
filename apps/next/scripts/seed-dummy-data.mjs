...
const orderIds = {
  ...
  refundPending: fixtureId("ord_refp1"),
  refunded: fixtureId("ord_refd1"),
};
...
const paymentIds = {
  ...
  refundPending: fixtureId("pay_refp1"),
  refunded: fixtureId("pay_refd1"),
};
...
const menuOrderIds = {
  ...
  refundPending: fixtureId("mo_refp1"),
  refunded: fixtureId("mo_refd1"),
};
...
liveColumns = {
  ...
  orders: await getExistingColumns("orders", ["id", "clientOrderId", "displayNumber", "status", "expiresAt", "cancelReason", "cancelledAt", "cancelledByUserId", "tableContextId", "paymentId", "userId", "createdAt", "updatedAt", "deletedAt"]),
  payments: await getExistingColumns("payments", ["id", "paid", "amount", "method", "bank", "depositor", "orderId", "status", "paymentCode", "originalAmount", "expectedTransferAmount", "expiresAt", "paidAt", "matchedBankTransactionId", "matchedBy", "depositorHint", "refundAmount", "refundRequestedAt", "refundedAt", "refundHandledByUserId", "refundNote", "userId", "createdAt", "updatedAt", "deletedAt"]),
...
await upsertRows("orders", [
  ...
  {
    id: orderIds.refundPending,
    clientOrderId: "demo-client-refund-pending",
    displayNumber: 4,
    status: "CANCELLED",
    expiresAt: now - 28 * oneMinute,
    cancelReason: "고객 요청으로 환불 대기",
    cancelledAt: now - 20 * oneMinute,
    cancelledByUserId: userIds.admin,
    tableContextId: contextIds.family,
    createdAt: now - 35 * oneMinute,
    updatedAt: now - 20 * oneMinute,
    deletedAt: null,
  },
  {
    id: orderIds.refunded,
    clientOrderId: "demo-client-refunded",
    displayNumber: 5,
    status: "CANCELLED",
    expiresAt: now - 58 * oneMinute,
    cancelReason: "중복 주문 환불 완료",
    cancelledAt: now - 49 * oneMinute,
    cancelledByUserId: userIds.admin,
    tableContextId: contextIds.family,
    createdAt: now - 60 * oneMinute,
    updatedAt: now - 45 * oneMinute,
    deletedAt: null,
  },
]);
...
await upsertRows("payments", [
  ...
  {
    id: liveColumns.payments.includes("orderId") ? paymentIds.refundPending : orderIds.refundPending,
    paid: 1,
    amount: 18496,
    method: "국민은행 이서연",
    bank: "국민은행",
    depositor: "이서연",
    orderId: orderIds.refundPending,
    status: "REFUND_PENDING",
    paymentCode: 4,
    originalAmount: 18500,
    expectedTransferAmount: 18496,
    expiresAt: now - 28 * oneMinute,
    paidAt: now - 27 * oneMinute,
    matchedBankTransactionId: null,
    matchedBy: "MANUAL",
    depositorHint: "이서연",
    refundAmount: 18496,
    refundRequestedAt: now - 20 * oneMinute,
    refundedAt: null,
    refundHandledByUserId: null,
    refundNote: null,
    createdAt: now - 35 * oneMinute,
  ...
  {
    id: liveColumns.payments.includes("orderId") ? paymentIds.refunded : orderIds.refunded,
    paid: 0,
    amount: 6495,
    method: "국민은행 최민준",
    bank: "국민은행",
    ...
    paymentCode: 5,
    originalAmount: 6500,
    expectedTransferAmount: 6495,
    expiresAt: now - 58 * oneMinute,
    paidAt: now - 57 * oneMinute,
    matchedBankTransactionId: null,
    matchedBy: "MANUAL",
    depositorHint: "최민준",
    refundAmount: 6495,
    refundRequestedAt: now - 49 * oneMinute,
    refundedAt: now - 45 * oneMinute,
    refundHandledByUserId: userIds.admin,
    refundNote: "현장 계좌로 환불 완료",
    createdAt: now - 60 * oneMinute,
    updatedAt: now - 45 * oneMinute,
    deletedAt: null,
  },
